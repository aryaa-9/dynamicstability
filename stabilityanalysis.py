...
def ddtenergy(S):
    ...
    dfdx = sp.diff(energyfunc((x, y, c, k)), x)
    dfdy = sp.diff(energyfunc((x, y, c, k)), y)
    
    xdot=Sdot(t, (x, y, c, k))[0]
    ydot=Sdot(t, (x, y, c, k))[1]
...
def main():
    x, y, k, c, t = sp.symbols('x y k c t')
    
    #V(x,y)
    V = energyfunc((x, y, c, k))
    #sp.pretty_print(V)
    
    dSdt = Sdot(t, (x, y, c, k))
    #sp.pretty_print(dSdt)

    sol1 = equipts(dSdt, (x, y, c, k))
    l = len(sol1)
    ...
    for i in range(l):
        print(f'V{sol[i]} = ', evalV(V, (x, y, c, k), sol, l)[i])
    
    x.evalf()
    y.evalf()
    dVdt = ddtenergy((x, y, c, k, t))
    f = sp.lambdify((x, y, k, c), dVdt, 'numpy')
